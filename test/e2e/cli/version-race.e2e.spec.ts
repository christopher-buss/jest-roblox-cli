...
describe("optimistic place-version pinning", () => {
	it("should retry a raced task pinned to the uploaded version and still pass", async () => {
		...
		const server = await startFakeOpenCloudServerAsync([
			{ rawOutput: formatPlaceMismatch(2) },
		...
		expect(server.requests[0]!.script).toContain(placeIdentityGuardSource({ placeVersion: 1 }));
		...
		expect(server.requests[1]!.script).not.toContain(PLACE_MISMATCH);
	});
});
