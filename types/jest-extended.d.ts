...
// parameter, while vitest declares `Matchers<R, T = {}>` and passes the asserted
// value's type as `T`. Merging the two is legal (the extra parameter has a
// default) but the merged interface keeps the one-parameter list, so `T` never
// substitutes and every matcher falls back to `any`/`unknown`. Augmenting can't
// fix that: merged members become overloads, and the package's permissive
// `<E = unknown>(members: readonly E[])` signatures still match anything, so a
// wrong argument silently resolves against them instead of erroring.
//
...
/**
 * `.resolves` / `.rejects` keep `T` as the promise itself rather than its
 * settled value, so matcher arguments degrade to `unknown` there.
 */
...
declare namespace jest {
	// `{}` is vitest's own default for this parameter. Merged interface
	// declarations must have identical type parameters, defaults included, so
	// anything else here (`object`, `unknown`) fails with TS2428 and drops `T`.
	// eslint-disable-next-line ts/no-empty-object-type -- must match vitest
...
declare module "jest-extended" {
	const matchers: Parameters<typeof import("vitest").expect.extend>[0];
	export = matchers;
}
