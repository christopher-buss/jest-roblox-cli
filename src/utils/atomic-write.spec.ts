 ...
 */
function captureTemporaryNames(): Array<string> {
 ...
 */
function refuseToDeleteOwner(owner: number): void {
...
describe(atomicWrite, () => {
	it("should write contents to the target path", () => {
		...
		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });
	...
	it("should create missing parent directories before writing", () => {
		...
		atomicWrite({ contents: "hello", targetPath: "/nested/deep/coverage/manifest.json" });
	...
	it("should replace an existing target on the last retry left", () => {
		...
		atomicWrite({ contents: "new", targetPath: "/coverage/manifest.json" });
	...
	it("should name the temp file after this process and the write's own nonce", () => {
		...
		const names = captureTemporaryNames();
	...
	it("should give two writes to one target different temp names", () => {
		...
		atomicWrite({ contents: "first", targetPath: "/coverage/manifest.json" });
		atomicWrite({ contents: "second", targetPath: "/coverage/manifest.json" });
	...
	it("should delete a stray temp file left by a process that is no longer running", () => {
		...
		vol.fromJSON({ [`/coverage/manifest.json.tmp.${STRAY_STAMP}`]: "abandoned" });
		reportDeadOwners([4242]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.existsSync(`/coverage/manifest.json.tmp.${STRAY_STAMP}`)).toBeFalse();
		expect(vol.readFileSync("/coverage/manifest.json", "utf-8")).toBe("hello");
	});
	...
	it("should keep the temp file of a peer that is still running", () => {
		...
		vol.fromJSON({ [`/coverage/manifest.json.tmp.${STRAY_STAMP}`]: "in flight" });
		reportDeadOwners([]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.readFileSync(`/coverage/manifest.json.tmp.${STRAY_STAMP}`, "utf-8")).toBe(
			"in flight",
		);
	});
	...
	it("should keep a temp file whose owner refuses the liveness probe", () => {
		...
		vol.fromJSON({ [`/coverage/manifest.json.tmp.${STRAY_STAMP}`]: "another user write" });
	...
	it("should leave a sibling target's stray temp file alone", () => {
		...
		vol.fromJSON({ [`/coverage/place.json.tmp.${STRAY_STAMP}`]: "another target" });
		reportDeadOwners([4242]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/cache.json" });

		expect(vol.readFileSync(`/coverage/place.json.tmp.${STRAY_STAMP}`, "utf-8")).toBe(
			"another target",
		);
	});
	...
	it("should leave a sibling alone when more follows the nonce", () => {
		...
		vol.fromJSON({ [`/coverage/manifest.json.tmp.${STRAY_STAMP}.bak`]: "not a temp file" });
		reportDeadOwners([4242]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.readFileSync(`/coverage/manifest.json.tmp.${STRAY_STAMP}.bak`, "utf-8")).toBe(
			"not a temp file",
		);
	});
	...
	it("should leave a sibling alone when the stamp opens with more than a pid", () => {
		...
		vol.fromJSON({ [`/coverage/manifest.json.tmp.pid${STRAY_STAMP}`]: "not a temp file" });
		reportDeadOwners([4242]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.readFileSync(`/coverage/manifest.json.tmp.pid${STRAY_STAMP}`, "utf-8")).toBe(
			"not a temp file",
		);
	});
	...
	it("should leave a temp file carrying no nonce alone", () => {
		...
		vol.fromJSON({ "/coverage/manifest.json.tmp.4242": "left by an older version" });
		reportDeadOwners([4242]);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.readFileSync("/coverage/manifest.json.tmp.4242", "utf-8")).toBe(
	...
	it("should keep sweeping past a stray temp file it cannot delete", () => {
		...
		refuseToDeleteOwner(4242);

		atomicWrite({ contents: "hello", targetPath: "/coverage/manifest.json" });

		expect(vol.existsSync("/coverage/manifest.json.tmp.4243.a1b2c3d4")).toBeFalse();
	});
