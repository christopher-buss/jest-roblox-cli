...
describe(runMultiProjectAsync, () => {
	...
	/**
	 * The bundle path an open-cloud place build would write beside the place
	 * it split from, which is `place-builder`'s rule rather than a path any
	 * caller here names. No production run asks for one.
	 */
	...
	it("should build the whole open-cloud place", async () => {
		...
		expect(writtenSynthProject(volume).tree).toStrictEqual({
			...
			ReplicatedStorage: {
				client: {
					"$path": "../../out/client",
					"jest.config": { $path: "out/client/jest.config.luau" },
				},
			},
			ServerScriptService: {
				server: {
					"$path": "../../out/server",
					"jest.config": { $path: "out/server/jest.config.luau" },
				},
			},
		});
	});

	it("should ask the coverage build for no harness on open cloud", async () => {
		expect.assertions(1);
	...
	it("should hand the backend no Code Bundle", async () => {
		expect.assertions(1);

		await runCoverageAsync("open-cloud");

		expect(mocks.runProjects.mock.calls[0]![0].codeBundle).toBeUndefined();
	});
